...
export default function StudyClassDetailsPage({
  ...
  const {
    data: studyClass,
    isLoading: isStudyClassLoading,
    error: studyClassError,
  } = useGetStudyClass(id);
  ...
  } = useGetSubscriptionsByStudyClass(id);

  const isLoading = isStudyClassLoading || isSubscriptionsLoading;
  const error = studyClassError || subscriptionsError;
